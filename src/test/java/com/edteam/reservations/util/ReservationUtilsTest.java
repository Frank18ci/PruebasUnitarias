...
public class ReservationUtilsTest {
    @Tag("success-case")
    @DisplayName("should return the information of the util")
    @Test
    void getReservation_should_return_the_information() {
        // Given

        // When
        Reservation result = ReservationUtils.getReservation(1L, "MIA", "EZE");
    ...
    void getReservation_should_return_the_information_with_mocks() {
        try (MockedStatic<ReservationUtils> utilities = Mockito.mockStatic(ReservationUtils.class)) {
            // Given
            utilities.when(() -> ReservationUtils.getReservation(1L, "BUE", "MIA"))
                    .thenReturn(getReservation(2L, "BUE", "MIA"));
            // When
            Reservation result = ReservationUtils.getReservation(1L, "BUE", "MIA");
            Segment segment = result.getItinerary().getSegment().get(0);
    ...
    static Reservation getReservation(Long id, String origin, String destination) {
